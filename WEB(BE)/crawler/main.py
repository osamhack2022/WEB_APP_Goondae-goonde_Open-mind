...
def mou_result(mou_data, temp):
    return print("mou크롤링 완료(크롤링 한 mou데이터 %d개, 총 데이터 %d개)" %(len(temp), len(mou_data)))
...
13. changwonsi\n14. nonsansi\n15. yeongcheonsi\n###MOU###\n99. MOUdata''')
    ...
    if select.isdigit():
        if select == "0":
            ...
            mou_result(crawling.crawled_mou_data, temp)
    if select == 'exit':
        break
